...
onglets.forEach((onglet) => {
  ...
  onglet.addEventListener("click", () => {
    ...
    //Ici on fait une boucle qui s'arrête lorsqu'on dépasse le nombre d'élément dans le tableau 'onglets'. Cette boucle va permettre de parcourir tous les 'onglet' et de prendre la valeur de l'élément "data-anim". Si la valeur est diférente de celle de l'index alors on enlève la class 'displayed'. Ceci va permettre d'afficher en couleur uniquement l'onglet sélectionné
    for (i = 0; i < onglets.length; i++) {
      if (onglets[i].getAttribute("data-anim") != index) {
        onglets[i].classList.remove("displayed");
      }
    }
    //Même raisonnement avec le contenu 
